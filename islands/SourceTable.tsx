...
export default function SourceTable({ relevantSources, angle, literatureReviewResults }: SourceTableProps) {
    ...
    const handleSelectedSource = (index: number) => {
        if (selectedSources.includes(index)) {
            const newSelectedSources = selectedSources.filter(sourceIndex => sourceIndex !== index)
        ...
        <div class="flex flex-col w-full mt-8">
            ...
            <table class="table-auto">
                <tr class="rounded-lg bg-black text-white p-2">
                    <th><p class="p-2">Select</p></th>
                    <th><p class="p-2">Title</p></th>
                    <th><p class="p-2">Author(s)</p></th>
                    <th><p class="p-2">Summary</p></th>
                    <th><p class="p-2">Link</p></th>
                    <th><p class="p-2">Relevance</p></th>
                </tr>
                {
                    relevantSources.value.map((source, i) => (
                        <tr class="my-2">
                            <td><input type="checkbox" onClick={() => handleSelectedSource(i)} checked={selectedSources.includes(i)}></input></td>
                            <td><p class="p-2">{source.sourceTitle}</p></td>
                            <td><p class="p-2">{source.sourceAuthors}</p></td>
                            <td><p class="p-2">{source.sourceSummary}</p></td>
                            <td><a href={source.sourceLink} target="_blank" rel="noopener noreferrer" class="p-2 text-blue-400">PDF</a></td>
                            <td><p class="p-2">{source.relevance}</p></td>
                        </tr>
                    ))
                }
            </table>
